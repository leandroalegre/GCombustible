	...
	<table class="table table-striped" id="tablapersonas">
		...
		<tbody>
			<?php foreach ($per as $p): ?>
				<tr>
                <td style="vertical-align: middle; font-size: 15px"><img src=" <?php echo base_url()?>public/imagenes_personas/<?php echo $p->foto_persona; ?>" class="img-thumbnail" style="width: 60px; height: 60px "/></td>
					...
					<td style="vertical-align: middle; font-size: 15px"><button type="button" class="btn btn-lg btn-success btn-check" value="<?php echo $p->id."+".$p->nombre."+".$p->legajo."+".$p->dni."+".$p->dif;?>"><i class="glyphicon glyphicon-check"></i></button></td>
				</tr>
			<?php endforeach ?>
		</tbody>
	</table>
